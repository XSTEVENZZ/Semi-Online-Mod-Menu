...
public class Auth extends AsyncTask<String, Void, String> {
    ...
    // your native mthods is here
    // public native void Check();
    public native String Title();
	public native String CurrentStatusLib();
    private  String URLSERVER = Title();
Prefs prefs;

    public Auth(MainActivity activity){
    ...
    protected void onPostExecute(String s) {
        ...
        try {
            ...
            if(data.get("Status").toString().equals("Success")) {
                ...
                String vend = prefs.write(VENDEDOR, data.get("Vendedor").toString());
                String val = prefs.write(VAL, data.get("SubscriptionLeft").toString());
				String title = prefs.write(TITLE, data.get("Title").toString());
				String icon = prefs.write(ICON, data.get("icon").toString());
				String isactive = prefs.write(ISACTIVE, data.get("isactive").toString());
				String feature0 = prefs.write(FEATURE0, data.get("feature0").toString());
				String feature1 = prefs.write(FEATURE1, data.get("feature1").toString());
				String feature2 = prefs.write(FEATURE2, data.get("feature2").toString());
				
			   String CurrentStatus = data.get("isactive").toString();
			   if (CurrentStatus.toString().equals(CurrentStatusLib())) {
				   AlertDialog.Builder alertDialog = new AlertDialog.Builder(activity)
					   .setTitle("!Conta")

					   // .setMessage("ok");
					   .setMessage("Bem Vindo:\nUsuario:" + data.get("Username").toString() + "\nValidade:" + data.get("SubscriptionLeft").toString().replace(" 00:00:00", "") + "\nData de Registro: " + data.get("RegisterDate").toString().replace(" 00:00:00", "") + "\nVendedor: " + data.get("Vendedor").toString() + "\nStatus da Conta: Ativado!");
				   alertDialog.setNegativeButton("OKAY!", new DialogInterface.OnClickListener() {
						   ...
						   public void onClick(DialogInterface dialog, int which) {
							   ...
							   if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !Settings.canDrawOverlays(activity)) {
								   ...
								   Intent i = new Intent(activity, FloatingModMenuService.class);
								   // static String s1 = "";
								   try {
									   Toast.makeText(activity,Html.fromHtml("<font color='#FFFF00'>" + data.get("Dias").toString() + "</font>"), Toast.LENGTH_LONG).show();
								   } catch (JSONException e) {
									   e.printStackTrace();
								   }
								   try {
									   Intent intent = new Intent(getActivity(), Class.forName("uk.lgl.modmenu.MainActivity"));
				   ...
				   AlertDialog.Builder alertDialogb= new AlertDialog.Builder(activity)
					   ...
					   // .setMessage("ok");
					   .setMessage("Mod Menu Desativado!");
				   alertDialogb.setNegativeButton("OKAY!", new DialogInterface.OnClickListener(){
					   @Override
					   public void onClick(DialogInterface dialog, int which) {
                ...
                //alert dialog for validade
                
                Toast.makeText(activity, (Html.fromHtml("<font face='monospace'> <font color='#00ff00'>Logado com Sucesso!</font></font>")), Toast.LENGTH_LONG).show();
                //getActivity().startActivity(new Intent(activity, Class.forName(getActivity().sGameActivity)));
                //System.loadLibrary("MyLibName");
                // Check();
                if (data.get("Status").toString().equals("Success")) {

                    //dont put nothing here....
                    ...
                    if (!isServiceRunning()) {
                        ...
                        Toast.makeText(activity, "Dont crack it pls", Toast.LENGTH_SHORT).show();
                        getActivity().finishAffinity();
                    }
                }
                //getActivity().finish();
    ...
    private boolean isServiceRunning() {
        if (a != "https://meuserver.com/login.php") {
            return true;
        } else {
            return false;
        }
